...
# from phrase_ble.serializer import VerbSerializer, PronounSerializer
...
def get_random_verb_pair(request):
    ...
    en_swa = [words.strip().split("\t") for words in word_dict]
    random_word = random.choice(en_swa)
    return JsonResponse({'en': random_word[1], 'foreign': random_word[2]})
...
def get_en_swa_translation(request):
    ...
    swahili_1 = swahili_translation[len(
        swahili_translation)//3:2*len(swahili_translation)//3]
...
def get_swa_right_verb(request):
    ...
    while(1):
        ...
        if(feat not in feats):
            feats = feats + [feat]
            swahili_translations = swahili_translations + \
                [translate_swahili(feat)]
        if(len(feats) == 4):
            break
...
def get_feats(word, sp, tense, neg):
    feats = random.choice(neg) + random.choice(sp) + \
        "+" + random.choice(tense) + "+" + word
    return feats
...
def translate(word, feats, src, tgt):
    ...
    df = pd.read_csv("data/test_spanish_sentences.tsv",
